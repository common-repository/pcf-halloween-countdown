...
function pcfch_calculate($day, $month, $year){
...
function pcfch_countdown($id, $type){
    $day = 31;
    $month = 10;
    $year = date("Y");
    
    list($days, $weeks, $day_remainder, $sleeps) = pcfch_calculate($day, $month, $year);
...
function pcfch_shortcode($atts){
    ...
    if(empty($atts['id'])){
        pcfch_countdown('', $atts['type']);
    }
    else{
        pcfch_countdown($atts['id'], $atts['type']);
    }
    
    return ob_get_clean();
}

add_shortcode('pcf_hwn_countdown', 'pcfch_shortcode');
